    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-center">
            ...
            <div class="lg:col-span-7 space-y-6 text-right">
                ...
                <div class="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-emerald-950/80 border border-gold-400/40 text-gold-300 text-xs font-medium backdrop-blur-sm shadow-md">
                ...
                <p class="text-sm md:text-base text-slate-200 leading-relaxed max-w-2xl font-light">
                    ...
                    @else
                        المنصة العامة والفكرية المخصصة للمحاضرات العامة، مؤلفات الفكر الإسلامي، ديوان الشعر، الفتاوى والاستفسارات، والأنشطة التبليغية في العالم.
                    @endif
                </p>
                ...
                <div class="flex flex-wrap items-center gap-3 pt-2">
                    @if($featuredArticle)
                        <a href="{{ route('news.show', $featuredArticle->slug) }}" class="px-6 py-3 rounded-xl bg-gradient-to-r from-gold-400 via-gold-500 to-gold-600 hover:from-gold-300 hover:to-gold-500 text-emerald-950 font-bold text-sm shadow-xl hover:shadow-gold-500/20 transition-all transform active:scale-95 flex items-center gap-2">
            ...
            <div class="lg:col-span-5 flex justify-center">
                <div class="relative w-full max-w-sm">
                    ...
                    <div class="absolute -inset-2 bg-gradient-to-r from-gold-500/40 via-emerald-500/30 to-gold-600/40 rounded-[2.5rem] blur-xl opacity-60"></div>
...
<section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-6 relative z-20">
    <div class="grid grid-cols-1 md:grid-cols-12 gap-6">
        ...
        <div class="md:col-span-7 bg-white rounded-2xl p-6 border border-slate-200 shadow-lg flex items-center gap-5">
            <div class="w-14 h-14 rounded-2xl bg-gold-50 text-gold-600 flex items-center justify-center text-2xl flex-shrink-0 border border-gold-200">
                <i class="fa-solid fa-quote-right"></i>
            </div>
            <div class="space-y-1 flex-grow">
                <span class="text-[11px] font-bold uppercase tracking-wider text-emerald-800 bg-emerald-50 px-2.5 py-0.5 rounded-full">
        ...
        <div class="md:col-span-5 bg-gradient-to-br from-emerald-950 to-emerald-900 rounded-2xl p-6 border border-gold-500/40 shadow-lg text-white flex flex-col justify-between">
            <div class="flex items-center justify-between">
                <span class="px-2.5 py-0.5 rounded-full bg-gold-500/20 text-gold-300 text-[11px] font-bold border border-gold-500/30">
            ...
            <div class="py-2">
                ...
                <p class="text-xs text-slate-300 mt-0.5">أبحاث الفقه والأصول والتفسير، المقررات، وجداول الحلقات العلمية.</p>
            </div>
            <a href="{{ $hawzaPortalUrl }}" target="_blank" class="text-xs text-gold-300 font-bold hover:underline flex items-center gap-1">
...
<section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
    <div class="flex flex-wrap items-end justify-between gap-4 mb-8">
        <div>
            <h2 class="text-2xl sm:text-3xl font-bold font-scholarly text-slate-900">جديد المحاضرات والمرئيات</h2>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm hover:shadow-md transition flex flex-col justify-between group">
            <div>
                ...
                <div class="relative bg-black aspect-video flex items-center justify-center overflow-hidden">
                    <img src="{{ $yt->thumbnail }}" alt="{{ $yt->title }}" class="w-full h-full object-cover group-hover:scale-105 transition duration-300">
                    <a href="{{ $yt->url }}" target="_blank" class="absolute inset-0 flex items-center justify-center bg-black/40 group-hover:bg-black/20 transition">
            ...
            <div class="p-4 bg-slate-50 border-t border-slate-100 flex items-center justify-between text-xs">
                <a href="{{ $yt->url }}" target="_blank" class="font-bold text-red-600 hover:text-red-700 flex items-center gap-1">
    ...
    <!-- Audios Subsection with Instant Floating Player -->
    @if($featuredAudios->count() > 0)
    <div class="border-t border-slate-200 pt-10">
        <div class="flex items-center justify-between mb-6">
            <div class="flex items-center gap-2">
                <i class="fa-solid fa-headphones text-gold-600"></i>
                <h3 class="text-lg font-bold font-scholarly text-slate-900">جديد المحاضرات الصوتية (استماع فوري)</h3>
            </div>
            <a href="{{ route('audios.index') }}" class="text-xs font-bold text-emerald-800 hover:underline">المكتبة الصوتية كاملة ←</a>
        </div>

        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            @foreach($featuredAudios as $audio)
            <div class="bg-white rounded-2xl p-5 border border-slate-200 shadow-sm hover:shadow-md transition flex flex-col justify-between space-y-4">
                <div>
                    <div class="flex items-center justify-between text-xs text-slate-400 mb-2">
                        <span class="px-2 py-0.5 rounded bg-emerald-50 text-emerald-800 font-semibold">{{ $audio->season_year ?? 'محاضرة عامة' }}</span>
                        <span><i class="fa-regular fa-clock"></i> {{ $audio->duration ?? '40:00' }}</span>
                    </div>
                    <h4 class="font-bold text-sm text-slate-800 leading-snug line-clamp-2 hover:text-emerald-800 transition">
                        <a href="{{ route('audios.show', $audio->slug) }}">{{ $audio->title }}</a>
                    </h4>
                </div>

                <div class="pt-3 border-t border-slate-100 flex items-center justify-between">
                    <button onclick="playGlobalAudio('{{ $audio->media_url }}', '{{ addslashes($audio->title) }}')" class="px-3 py-1.5 rounded-lg bg-emerald-800 hover:bg-emerald-900 text-gold-300 text-xs font-semibold flex items-center gap-1.5 shadow transition">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
            ...
            <div class="lg:col-span-7 space-y-6">
                <div class="flex items-center justify-between">
                    ...
                    <a href="{{ route('books.index') }}" class="text-xs font-bold text-emerald-800 hover:underline">عرض كل الكتب ←</a>
                </div>

                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    @forelse($featuredBooks as $book)
                    <div class="bg-white rounded-2xl p-4 border border-slate-200 shadow-sm flex gap-4 hover:shadow-md transition">
                        <!-- Book Icon / Cover placeholder -->
                        <div class="w-16 h-24 rounded-lg bg-emerald-900 text-gold-300 flex items-center justify-center flex-shrink-0 shadow text-2xl font-scholarly">
                            <i class="fa-solid fa-book"></i>
                        </div>
                        <div class="flex flex-col justify-between min-w-0">
                            <div>
                                <h4 class="font-bold text-xs sm:text-sm text-slate-800 truncate" title="{{ $book->title }}">{{ $book->title }}</h4>
                                <p class="text-[11px] text-slate-400 mt-0.5">{{ $book->publisher ?? 'دار النشر' }} ({{ $book->publication_year ?? '2024' }})</p>
                                <p class="text-xs text-slate-500 line-clamp-2 mt-1 font-light leading-relaxed">{{ $book->summary }}</p>
                            </div>
                            <div class="flex items-center gap-2 pt-2">
                                <a href="{{ route('books.show', $book->slug) }}" class="text-[11px] font-bold text-emerald-800 hover:underline">قراءة وتنزيل ←</a>
                            </div>
                        </div>
                    </div>
                    @empty
            ...
            <div class="lg:col-span-5 space-y-6">
                <div class="flex items-center justify-between">
                    ...
                    <a href="{{ route('poems.index') }}" class="text-xs font-bold text-emerald-800 hover:underline">عرض الديوان ←</a>
                </div>

                @if($featuredPoem)
                <div class="bg-white rounded-2xl p-6 border border-gold-500/30 shadow-md space-y-4">
                    <div class="flex items-center justify-between border-b border-slate-100 pb-3">
                        <span class="text-xs font-semibold px-2.5 py-0.5 rounded bg-gold-50 text-gold-700">{{ $featuredPoem->occasion ?? 'في رثاء سيد الشهداء (ع)' }}</span>
                    ...
                    <div class="pt-2 text-center">
                        <a href="{{ route('poems.show', $featuredPoem->slug) }}" class="inline-flex items-center gap-1.5 text-xs font-bold text-emerald-800 hover:text-emerald-950">
...
<section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
    <div class="flex items-center justify-between mb-8">
        <div>
            ...
            <h2 class="text-2xl sm:text-3xl font-bold font-scholarly text-slate-900">أخبار ونشاطات وجولات سماحة السيد</h2>
        </div>
        <a href="{{ route('news.index') }}" class="text-xs font-bold text-emerald-800 hover:underline">أرشيف الأخبار ←</a>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <article class="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm hover:shadow-md transition flex flex-col justify-between">
            ...
            <div class="p-4 bg-slate-50 border-t border-slate-100 flex items-center justify-between text-xs text-slate-400">
                <span><i class="fa-regular fa-calendar"></i> {{ $news->published_at ? $news->published_at->format('Y-m-d') : '' }}</span>
                <a href="{{ route('news.show', $news->slug) }}" class="font-semibold text-emerald-800 hover:text-emerald-950">التفاصيل ←</a>
            </div>
        </article>
        @empty
...
<section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-16">
    <div class="bg-gradient-to-r from-emerald-900 to-emerald-950 rounded-3xl p-8 sm:p-12 text-white shadow-xl flex flex-col md:flex-row items-center justify-between gap-8 border border-gold-500/30">
        <div class="space-y-3 max-w-xl text-right">
            ...
            <h2 class="text-2xl sm:text-3xl font-bold font-scholarly text-white">هل لديك استفسار فكري أو مسألة فقهية؟</h2>
            <p class="text-xs sm:text-sm text-slate-300 font-light leading-relaxed">
                يمكنكم إرسال استفساراتكم ومسائلكم الشرعية والفكرية ليتولى مكتب سماحة السيد الإجابة عنها، مع إمكانية متابعة حالة السؤال برقم التتبع الخاص.
            </p>
        </div>

        <div class="flex flex-wrap items-center gap-3">
            <a href="{{ route('inquiries.index') }}" class="px-6 py-3.5 rounded-xl bg-gold-500 hover:bg-gold-400 text-emerald-950 font-bold text-xs shadow-lg transition flex items-center gap-2">
                <i class="fa-solid fa-pen-to-square"></i>
                <span>طرح استفسار جديد</span>
            </a>
            <a href="{{ route('inquiries.track') }}" class="px-5 py-3.5 rounded-xl bg-emerald-800/80 hover:bg-emerald-800 text-slate-200 border border-slate-600 text-xs font-semibold transition flex items-center gap-2">
