...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <form action="{{ route('admin.books.update', $book->id) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm space-y-5">
        @csrf 
        @method('PUT')
        ...
        <div>
            ...
            <textarea name="summary" rows="3" class="w-full text-xs rounded-xl border-slate-200 p-2.5 bg-slate-50 focus:bg-white focus:ring-1 focus:ring-emerald-700">{{ old('summary', $book->summary) }}</textarea>
        </div>

        <div>
            <label class="block text-xs font-semibold text-slate-700 mb-1">فهرس الموضوعات:</label>
            <textarea name="table_of_contents" rows="6" class="w-full text-xs rounded-xl border-slate-200 p-2.5 bg-slate-50 focus:bg-white focus:ring-1 focus:ring-emerald-700">{{ old('table_of_contents', $book->table_of_contents) }}</textarea>
        </div>
