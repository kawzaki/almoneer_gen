        ...
        <div class="bg-white rounded-3xl p-8 border border-slate-200 shadow-sm hover:shadow-md transition flex flex-col justify-between space-y-6">
            <div>
                <div class="flex items-center justify-between text-xs text-slate-400 mb-3">
                    <span class="px-2.5 py-0.5 rounded bg-gold-50 text-gold-700 font-semibold">{{ $poem->occasion ?? 'قصيدة ولائية' }}</span>
                    <span>{{ $poem->meter ?? 'بحر البسيط' }}</span>
                </div>
            ...
            <div class="pt-4 border-t border-slate-100 flex items-center justify-between">
                <span class="text-xs text-slate-400">{{ count($poem->couplets) }} أبيات</span>
