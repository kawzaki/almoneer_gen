...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-12">
    ...
    <div class="text-center max-w-3xl mx-auto space-y-3">
        <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-50 border border-emerald-200/60 text-emerald-800 text-xs font-bold">
        ...
        <p class="text-xs sm:text-sm text-slate-500 leading-relaxed font-light">
            تصفح المحاضرات الدينية والفكرية لسماحة العلامة السيد منير الخباز بحسب المواسم السنوية الهجرية المتكررة، متوفرة بثلاث صيغ متكاملة: البث المرئي، التسجيل الصوتي، والتفريغ النصي الموثق.
        </p>
    </div>
    ...
    <div class="space-y-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div class="bg-gradient-to-br from-emerald-950 via-emerald-900 to-slate-950 text-white rounded-3xl p-6 shadow-md border border-emerald-800/40 relative overflow-hidden flex flex-col justify-between group">
                <div class="space-y-4 relative z-10">
                    <div class="flex items-center justify-between border-b border-emerald-800/60 pb-3">
                        <div class="flex items-center gap-2">
                            <span class="w-8 h-8 rounded-xl bg-gold-500 text-emerald-950 font-bold flex items-center justify-center text-sm shadow">
                                <i class="fa-solid fa-moon"></i>
                            </span>
                            <span class="text-xl font-bold font-scholarly text-gold-300">مواسم سنة {{ $yearNum }} هـ</span>
                        </div>
                ...
                <div class="pt-4 mt-4 border-t border-emerald-800/40 relative z-10">
                    <a href="{{ route('lectures.year', ['year' => $yearNum]) }}" class="w-full py-2 bg-emerald-800 hover:bg-gold-500 hover:text-emerald-950 text-white font-bold text-xs rounded-xl transition flex items-center justify-center gap-2">
    ...
    <div class="bg-white rounded-3xl p-5 border border-slate-200 shadow-sm space-y-4">
        ...
        <div class="pt-2 border-t border-slate-100 flex items-center justify-between">
            <div class="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-900 text-xs font-bold">
                <i class="fa-solid fa-tag text-gold-500"></i>
                <span>تصفية بحسب الوسم: #{{ $selectedTag }}</span>
            </div>
            <a href="{{ route('lectures.index', request()->except('tag')) }}" class="text-xs text-red-600 hover:text-red-700 font-semibold flex items-center gap-1">
        ...
        <div class="bg-white rounded-3xl border border-slate-200 overflow-hidden shadow-sm hover:shadow-md transition flex flex-col justify-between group">
            <div>
                ...
                <div class="relative bg-slate-900 aspect-video overflow-hidden">
                    ...
                    <!-- Season Pill Badge -->
                    <span class="absolute top-2 right-2 px-2.5 py-1 rounded-full bg-emerald-950/80 text-gold-300 text-[11px] font-bold border border-gold-400/20 backdrop-blur-sm">
                ...
                <div class="p-5 space-y-3">
                    <div class="flex items-center justify-between text-xs text-slate-400">
                        ...
                        <span><i class="fa-solid fa-eye ml-1 text-slate-400"></i> {{ number_format($lec->views_count) }}</span>
                    </div>
            ...
            <div class="p-4 bg-slate-50 border-t border-slate-100 flex items-center justify-between text-xs">
                <div class="flex items-center gap-2">
                    @if($lec->hasVideo())
                    <span class="px-2 py-0.5 rounded bg-red-100 text-red-700 text-[10px] font-bold flex items-center gap-1" title="متوفر مرئياً"><i class="fa-solid fa-video"></i> مرئي</span>
                    @else
                    <span class="px-2 py-0.5 rounded bg-slate-100 text-slate-500 text-[10px] flex items-center gap-1" title="المرئي قريباً"><i class="fa-solid fa-video-slash"></i> مرئي قريباً</span>
                    @endif

                    @if($lec->hasAudio())
                    <span class="px-2 py-0.5 rounded bg-amber-100 text-amber-800 text-[10px] font-bold flex items-center gap-1" title="متوفر صوتياً"><i class="fa-solid fa-headphones"></i> صوتي</span>
                    @endif

                    @if($lec->hasTranscript())
                    <span class="px-2 py-0.5 rounded bg-emerald-100 text-emerald-800 text-[10px] font-bold flex items-center gap-1" title="متوفر نصياً"><i class="fa-solid fa-file-lines"></i> مكتوب</span>
                    @endif
                </div>
    ...
    <div id="lectures-list-container" class="{{ $viewMode === 'list' ? 'space-y-4' : 'hidden' }} space-y-4">
        ...
        <div class="bg-white rounded-2xl border border-slate-200 p-4 sm:p-5 shadow-sm hover:shadow-md transition flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 group">
            ...
            <div class="flex items-start sm:items-center gap-4 w-full sm:w-auto">
                <!-- Thumbnail -->
                <div class="relative w-28 sm:w-36 aspect-video rounded-xl overflow-hidden bg-slate-900 shrink-0 shadow-sm">
                    <img src="{{ $lec->display_thumbnail }}" 
